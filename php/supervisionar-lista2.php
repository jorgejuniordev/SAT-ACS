<?php 
	if(isset($_GET["pg"])){
		...
		$sql2 = mysqli_query(conexao(), "SELECT * FROM usuarios WHERE status='ativa' AND tipo_de_conta='acs' AND cidade='$cidade' AND uf='$uf'") or die();
		$res2 = mysqli_num_rows($sql2);

		$sql = mysqli_query(conexao(), "SELECT * FROM usuarios WHERE status='ativa' AND tipo_de_conta='acs' AND cidade='$cidade'  AND uf='$uf' ORDER BY data_cadastro DESC LIMIT $pg, $limite") or die();
		$res = mysqli_num_rows($sql);

		if($res>=1){
			echo "<h3><center>LISTA DE CONTAS ATIVAS: ".ucfirst($db['cidade'])." / ".mb_strtoupper($db['uf'])."</center></h3>";
...
<?php
		...
		if($anterior<0){ // se anterior for menor que 0, ele exibe apenas os nomes sem link
			...
			echo "<a href=home.php?p=supervisionar-lista&list=2&pg=0>
					<button class=\"button btn\" type=\"button\">Primeira</button>
				 </a>";
			echo "<a href=home.php?p=supervisionar-lista&list=2&pg=$anterior>
		...
		if($prox>$res2){
			...
			echo "<a href=home.php?p=supervisionar-lista&list=2&pg=$prox>
					<button class=\"button btn\" type=\"button\">Proxima</button>
				 </a>";

			echo "<a href=home.php?p=supervisionar-lista&list=2&pg=$ultima>
